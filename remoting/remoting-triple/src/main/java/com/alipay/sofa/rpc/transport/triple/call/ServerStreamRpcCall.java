...
/**
 * Server-streaming (single request / multiple responses) RPC call implementation.
 *
 * <p>A {@code ServerStreamRpcCall} manages the lifecycle of a server-streaming RPC:
 * <ol>
 *   <li>Caller invokes {@link #start} with response headers and a listener</li>
 *   <li>Caller invokes {@link #sendMessage} one or more times with response data</li>
 *   <li>Caller invokes {@link #halfClose} when all response messages have been sent</li>
 * </ol>
 *
 ...
 * Exactly one request message is expected from the client.
 *
 ...
 */
public class ServerStreamRpcCall implements RpcCall, RpcStreamListener {
    ...
    /**
     * Creates a new {@code ServerStreamRpcCall} backed by the given stream.
     *
     * @param stream the underlying transport stream (must not be null)
     */
    public ServerStreamRpcCall(RpcStream stream) {
        this(stream, GrpcCodec.INSTANCE);
    }

    /**
     * Creates a new {@code ServerStreamRpcCall} with a custom codec (primarily for testing).
     *
     * @param stream the underlying transport stream
     * @param codec  the codec for encoding/decoding frames
     */
    public ServerStreamRpcCall(RpcStream stream, RpcCodec codec) {
        if (stream == null) {
    ...
    /**
     * Sends one response message. May be called multiple times for server-streaming.
     *
     * @param message the serialized response bytes
     */
    @Override
    ...
    public void onWritabilityChanged() {
        // Back-pressure notification — callers may pause sending when stream is not writable
    }
