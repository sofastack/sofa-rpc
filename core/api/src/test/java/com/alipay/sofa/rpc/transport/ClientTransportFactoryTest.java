...
public class ClientTransportFactoryTest {
    ...
    public void doReuseTest() {
        ...
        Assert.assertEquals(2, ClientTransportFactory.CLIENT_TRANSPORT_MAP.size());

        clientTransport2.setRequest(0);
        ClientTransportFactory.releaseTransport(clientTransport, 500);
        Assert.assertEquals(1, ClientTransportFactory.CLIENT_TRANSPORT_MAP.size());

        ClientTransportFactory.closeAll();

        Assert.assertEquals(0, ClientTransportFactory.CLIENT_TRANSPORT_MAP.size());
        Assert.assertTrue(ClientTransportFactory.ALL_TRANSPORT_MAP == null
            || ClientTransportFactory.ALL_TRANSPORT_MAP.size() == 0);
    }

    private void doNotReuseTest() {
        ClientTransportConfig config = new ClientTransportConfig();
        config.setProviderInfo(new ProviderInfo().setHost("127.0.0.1").setPort(12222))
            .setContainer("test");
        TestClientTransport clientTransport = (TestClientTransport) ClientTransportFactory.getClientTransport(config);
        ...
        clientTransport.setRequest(4);
        ClientTransportFactory.releaseTransport(clientTransport, 500);
        Assert.assertEquals(2, ClientTransportFactory.ALL_TRANSPORT_MAP.size());

        clientTransport2.setRequest(0);
        ClientTransportFactory.releaseTransport(clientTransport2, 500);
        Assert.assertEquals(1, ClientTransportFactory.ALL_TRANSPORT_MAP.size());

        ClientTransportFactory.closeAll();
        Assert.assertEquals(0, ClientTransportFactory.CLIENT_TRANSPORT_MAP.size());
        Assert.assertEquals(0, ClientTransportFactory.ALL_TRANSPORT_MAP.size());
    }

    @Test
    public void getClientTransport2() throws Exception {
        // 由于默认是长连接复用的，这里测试不复用的场景
        Field field = ClientTransportFactory.class.getDeclaredField("CHANNEL_REUSE");
        Field modifiers = field.getClass().getDeclaredField("modifiers");
        modifiers.setAccessible(true);
        modifiers.setInt(field, field.getModifiers() & ~Modifier.FINAL);//fianl标志位置0  
        field.setAccessible(true);

        Field field2 = ClientTransportFactory.class.getDeclaredField("ALL_TRANSPORT_MAP");
        Field modifiers2 = field2.getClass().getDeclaredField("modifiers");
        modifiers2.setAccessible(true);
        modifiers2.setInt(field2, field2.getModifiers() & ~Modifier.FINAL);//fianl标志位置0  
        field2.setAccessible(true);

        Boolean enable = (Boolean) field.get(null);
        ConcurrentHashMap map = (ConcurrentHashMap) field2.get(null);
        try {
            ConcurrentHashMap<ClientTransportConfig, ClientTransport> newMap = new ConcurrentHashMap<ClientTransportConfig, ClientTransport>();
            field.set(null, false);
            field2.set(null, newMap);
            doNotReuseTest();
        } finally {
